...
from pages.user_home_promoted_profiles_page import User_promoted_profiles
...
def test_home_screen_promoted_profiles(driver):
    ...
    home = User_promoted_profiles(driver)

    # ✅ Verify Promoted Profiles title
    ...
    # ✅ Scroll horizontally to reveal 'View More'
    assert (
    ...
    # ✅ Highlight & validate Profiles title
    title = home.highlight_profiles_title()
    assert "Profiles" in title, "❌ Profiles title not visible"

    # ✅ Click on Promoted card
    home.click_promoted_card()

    # ✅ Count all profile cards (keeps scrolling until none found)
    total_profiles = home.count_all_profile_cards()
    assert total_profiles > 0, "❌ No profiles found"
    print(f"🎉 Test Passed: Found total {total_profiles} profiles")
