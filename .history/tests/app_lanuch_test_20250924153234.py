...
def test_app_launch(driver):
    ...
    # Step 1: Wait for login screen (first element inside the app)
    ...
    assert app.wait_for_login_image_logo_check(), "❌ Login title not visible"
    print("🎉 Test Passed:  Login image  is visible")

    # Step 4: Print and verify login page text
    text = app.text_print_chack_login()
    assert text == "Enter registered mobile number and password ", "❌ Login page text mismatch"
